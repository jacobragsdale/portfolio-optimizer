...
def cap_single_name(data: PortfolioData, params: CapSingleNameParams) -> PortfolioData:
    ...
    if params.max_weight >= data.style.max_weight:
        return data.with_changes(rule="cap_single_name")
    return data.with_changes(rule="cap_single_name", style=data.style.model_copy(update={"max_weight": params.max_weight}))
...
def add_zero_alpha(data: PortfolioData) -> PortfolioData:
    ...
    if "alpha" in data.universe.columns:
        return data.with_changes(rule="add_zero_alpha")
    universe = data.universe.assign(alpha=pd.Series(0.0, index=data.universe.index, dtype="Float64"))
    return data.with_changes(rule="add_zero_alpha", universe=universe)
...
def restrict_low_liquidity(data: PortfolioData, params: LiquidityParams) -> PortfolioData:
    ...
    return data.with_changes(rule="restrict_low_liquidity", universe=universe)
...
def avoid_cross_portfolio_wash_sales(data: PortfolioData, ctx: SolveContext) -> PortfolioData:
    ...
    if not sold:
        return data.with_changes(rule="avoid_cross_portfolio_wash_sales")
    ...
    return data.with_changes(rule="avoid_cross_portfolio_wash_sales", universe=universe)
