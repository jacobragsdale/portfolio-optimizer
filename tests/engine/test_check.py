...
def test_the_resting_portfolio_verifies_when_it_is_feasible(make: Factories) -> None:
    spec = make.spec()
    report = verify(spec, make.solution(spec, objective=resting_objective(spec)), ChainState.empty(spec.security_ids), TERMS, CONSTRAINTS, profile=INFLOW)
...
def test_each_violation_is_detected(make: Factories, name: str, perturb: Perturbation) -> None:
    ...
    report = verify(spec, solution, ChainState.empty(spec.security_ids), TERMS, CONSTRAINTS, profile=INFLOW)
...
def test_a_group_limit_reads_its_band_and_tolerance_from_its_row(make: Factories) -> None:
    ...
    tight = verify(spec, make.solution(spec), ChainState.empty(spec.security_ids), TERMS, capped, profile=INFLOW)
    ...
    assert verify(spec, make.solution(spec, objective=resting_objective(spec)), ChainState.empty(spec.security_ids), TERMS, loose, profile=INFLOW).passed
...
def test_a_group_limit_naming_a_group_the_universe_lacks_is_refused(make: Factories) -> None:
    ...
    with pytest.raises(ConstraintSpecError, match=r"group\(s\) \['ENERGY'\]"):
        verify(spec, make.solution(spec), ChainState.empty(spec.security_ids), TERMS, [unknown], profile=INFLOW)
...
def test_a_violation_exactly_at_tolerance_passes_and_the_box_reports_where_it_binds(make: Factories) -> None:
    ...
    report = verify(spec, solution, ChainState.empty(spec.security_ids), (), (), profile=INFLOW, tolerances=Tolerances(violation=1e-6))
    ...
    binding = verify(all_cash, at_cap, ChainState.empty(spec.security_ids), (), (), profile=INFLOW)
...
def test_hash_mismatch_and_non_finite_values_fail(make: Factories) -> None:
    spec = make.spec()
    stale = verify(spec, make.solution(spec, spec_hash="0" * 64), ChainState.empty(spec.security_ids), TERMS, CONSTRAINTS, profile=INFLOW)
    assert "spec_hash_matches" in stale.violated
    broken = verify(spec, make.solution(spec, w=np.array([np.nan, 0.5, 0.5])), ChainState.empty(spec.security_ids), TERMS, (), profile=INFLOW)
...
def test_the_objective_gap_is_checked_and_a_registered_kind_is_recomputed_like_a_shipped_one(make: Factories) -> None:
    ...
    wrong_objective = verify(spec, make.solution(spec, objective=0.5), ChainState.empty(spec.security_ids), TERMS, CONSTRAINTS, profile=INFLOW)
    assert not wrong_objective.objective_passed
    terms = terms_of("alpha", {"kind": "quadratic", "name": "risk", "column": "variance"})
    report = verify(spec, make.solution(spec, objective=resting_objective(spec) + 0.02 / 3.0), ChainState.empty(spec.security_ids), terms, CONSTRAINTS, profile=INFLOW)
...
def test_true_optimum_verifies_including_the_objective(make: Factories, frames: Frames) -> None:
    ...
    resolved = resolved_example(order_flow="outflow", objective=SELL_TERMS)
...
def test_verification_works_from_persisted_files(make: Factories, tmp_path: Path) -> None:
    ...
    assert verify(loaded_spec, loaded_solution, ChainState.empty(loaded_spec.security_ids), TERMS, constraints_of(loaded_solution), profile=INFLOW).passed
