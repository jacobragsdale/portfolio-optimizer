"""Tier 1/2: the shipped rules (boundary, empty, normal) and the pipeline's provenance and guards; rules never see other portfolios."""
...
def test_apply_rules_records_provenance_and_row_counts(make: Factories) -> None:
    ...
    result, audits = apply_rules(make.portfolio_data(), rules)  # ty: ignore[invalid-argument-type]  # resolve_step returns ResolvedStep
...
def test_apply_rules_rejects_a_rule_that_returns_the_wrong_type(make: Factories) -> None:
    with pytest.raises(RuleError, match="returned DataFrame, expected PortfolioData"):
        apply_rules(make.portfolio_data(), (step("tests.conftest:lying_rule"),))  # ty: ignore[invalid-argument-type]  # see above
