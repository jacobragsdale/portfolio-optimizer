...
from tests.engine.test_schedule import ids, overlap_graph
...
def test_every_edge_has_a_witness_security_and_every_overlap_has_an_edge(sets: list[frozenset[str]]) -> None:
    ...
    schedule = overlap_graph(order, {portfolio_id: tuple(sorted(members)) for portfolio_id, members in zip(order, sets, strict=True)})
