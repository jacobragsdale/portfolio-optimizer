...
def test_inflow_split_is_the_clipped_delta_and_no_sell() -> None:
    buy, sell = INFLOW.split(np.array([0.5, 0.3 - 1e-12, 0.3]), np.array([0.4, 0.3, 0.3]))
...
def test_inflow_identity_residuals_name_what_is_violated(make: Factories, w: np.ndarray, buy: np.ndarray, sell: np.ndarray, violated: set[str]) -> None:
    spec = make.spec(w0=np.array([0.4, 0.3, 0.3]))
    residuals = dict(INFLOW.identity_residuals(spec, make.solution(spec, w=w, buy=buy, sell=sell)))
...
def test_inflow_couples_through_buys_and_reports_only_buys(make: Factories, frames: Frames) -> None:
...
def test_inflow_names_the_starts_it_cannot_trade_out_of(make: Factories) -> None:
    fine = make.spec(w0=np.array([0.3, 0.3, 0.3]), cash_lb=0.0, cash_ub=0.1)
    assert INFLOW.infeasible_starts(fine) == []
    assert INFLOW.infeasible_starts(make.spec(w0=np.array([0.3, 0.3, 0.3]), scalars={})) == [], "a spec without cash bounds has no cash start to be wrong"
    low_cash = make.spec(w0=np.array([0.3, 0.3, 0.3]), cash_lb=0.2, cash_ub=0.5)
    assert INFLOW.infeasible_starts(low_cash) == ["the book starts with cash 0.100000 below cash_lb 0.200000, and an inflow can only lower cash"]
    over_cap = make.spec(w0=np.array([0.6, 0.3, 0.1]), ub=np.array([0.5, 0.3, 1.0]))
    assert INFLOW.infeasible_starts(over_cap) == ["names whose cap is below their holding, which this order flow cannot trade out of: ['S0']"], "S1 sits exactly at its cap, which is allowed"


# --- sell ---


def test_outflow_split_is_the_clipped_deficit_and_no_buy() -> None:
    buy, sell = OUTFLOW.split(np.array([0.3, 0.3 + 1e-12, 0.3]), np.array([0.4, 0.3, 0.3]))
...
def test_outflow_identity_residuals_name_what_is_violated(make: Factories, w: np.ndarray, buy: np.ndarray, sell: np.ndarray, violated: set[str]) -> None:
    spec = make.spec(w0=np.array([0.4, 0.3, 0.3]))
    residuals = dict(OUTFLOW.identity_residuals(spec, make.solution(spec, w=w, buy=buy, sell=sell)))
    assert set(residuals) == {"no_buys", "trade_balance", "nonneg_sell", "buy_absent", "lb", "ub"}
    assert {name for name, residual in residuals.items() if residual.max() > 1e-9} == violated


def test_outflow_couples_through_sells_and_reports_only_sells(make: Factories, frames: Frames) -> None:
...
def test_outflow_names_the_starts_it_cannot_trade_out_of(make: Factories) -> None:
    fine = make.spec(w0=np.array([0.3, 0.3, 0.3]), cash_lb=0.0, cash_ub=0.1)
    assert OUTFLOW.infeasible_starts(fine) == []
