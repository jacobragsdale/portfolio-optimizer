...
|---|---|---|
| `run CONFIG` | `--as-of DATETIME` (required: an ISO 8601 instant with a zone, e.g. `2026-08-28T00:00:00Z`), `--data-root PATH`, `--output PATH`, `--max-workers N`, `--retry-of MANIFEST`, `--retry-stages STAGES`, `--retry-errors ERRORS` | Load, solve, verify, publish, and write the manifest. Prints the run id, the manifest path, and one line per portfolio naming the constraints that bind (`binding: ...`). With `--retry-of`, the config runs over exactly the portfolios that manifest recorded as failed at the stages `--retry-stages` names (comma-separated from `load, slice, build, solve, worker, skipped`; default `solve`) and, when `--retry-errors` is given, with one of those exception types (`InfeasibleError,VerificationError`), in their solve order, written inline as the book, and the run is tagged `retry_of` with that run's id; refused (exit `2`) when nothing in the manifest matches, naming what did fail, or when a selector is given without a manifest. |
