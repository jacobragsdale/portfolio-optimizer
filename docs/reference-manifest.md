...
|---|---|---|
| `run CONFIG` | `--as-of DATETIME` (required: an ISO 8601 instant with a zone, e.g. `2026-08-28T00:00:00Z`), `--data-root PATH`, `--output PATH`, `--max-workers N`, `--retry-of MANIFEST` | Load, solve, verify, publish, and write the manifest. Prints the run id, the manifest path, and one line per portfolio naming the constraints that bind (`binding: ...`). With `--retry-of`, the config — which must be `order_flow: rebalance` — runs over exactly the portfolios that manifest recorded as failed at stage `solve`, in their solve order, written inline as the book, and the run is tagged `retry_of` with that run's id; refused (exit `2`) when the config is another order flow or nothing in the manifest failed at solve. |
