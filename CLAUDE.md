...
- The README's annotated `jsonc` block must parse equal to `configs/example_buy.json`; a test enforces
  it, so update both together. `configs/example_sell.json` is the same wiring for the sell program.
